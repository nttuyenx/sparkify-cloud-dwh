...
class DataQualityOperator(BaseOperator):
    ...
    def check_rows(self, redshift_hook, table):
        records = redshift_hook.get_records(
            """
            SELECT COUNT(*) FROM {}
            """.format(table))
        if len(records) < 1 or len(records[0]) < 1:
            raise ValueError("Data quality check failed, {} table returned \
                             no results".format(table))
        num_records = records[0][0]
        if num_records < 1:
            raise ValueError("Data quality check failed, {} table contained \
                             0 rows".format(table))
        self.log.info("Having rows check on table {} passed with {} records"
                      .format(table, num_records))

    def check_null(self, redshift_hook, table):
        ...
        if len(records) > 0:
            raise ValueError("Data quality check failed, {} table contains \
                             NULL values".format(table))
    ...
    def execute(self, context):
        ...
        for table in self.tables:
            self.check_rows(redshift_hook, table)

        for table in self.tables:
            self.check_null(redshift_hook, table)
